...
`;
    ...
    let fileName;
    // --- НОВОЕ: Переменная для "чистого" названия (без ZID) ---
    ...
    if (zidMatch) {
        // СЛУЧАЙ А: ZID найден.
        new Notice("Обнаружен ZID. Использую его.", 2000);
        const existingZid = zidMatch[1];
        const textAfterZid = zidMatch[2].trim();
        
        // --- ИЗМЕНЕНИЕ: Используем текст ПОСЛЕ ZID как чистое название ---
        cleanTaskName = textAfterZid;

        const words = textAfterZid.split(/\s+/);
        const firstThreeWords = words.slice(0, 3);
        const safeTaskName = firstThreeWords.join('-').toLowerCase().replace(/[^a-z0-9-а-яё]/g, '');
        
        fileName = `${existingZid}-${safeTaskName}`;

    } else {
        // СЛУЧАЙ Б: ZID не найден. Работаем по старой схеме.
        const words = originalTaskName.split(/\s+/);
        const firstThreeWords = words.slice(0, 3);
        const safeTaskName = firstThreeWords.join('-').toLowerCase().replace(/[^a-z0-9-а-яё]/g, '');
    ...
    // --- ИЗМЕНЕНИЕ: Используем cleanTaskName для алиаса ---
    const aliasesYamlBlock = `\n  - ${cleanTaskName}`;
...
`;
    ...
    if (editor) {
        // --- ИЗМЕНЕНИЕ: Вставляем ссылку с чистым алиасом ---
