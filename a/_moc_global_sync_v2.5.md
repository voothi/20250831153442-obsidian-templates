...
--------------------------------------------------------------------------------
 ...
 !!! НОВОЕ в v2.5 !!!
 1. (ТОЧНОЕ ПРАВИЛО ДЛЯ ЗАГОЛОВКА) Исправлена ошибка в регулярном выражении.
    Теперь скрипт обрабатывает секцию, только если слово "MOC" находится
    в самом конце заголовка. Допускаются только точки и пробелы после него.
    Это финальная и наиболее точная версия правила.
================================================================================
*/

async function globalMocSyncV2(tp) {
  // >>>>>>>>>> ГЛАВНОЕ ИЗМЕНЕНИЕ ЗДЕСЬ <<<<<<<<<<
  const MOC_HEADER_REGEX = /^(#+)\s+(.*\bMOC\b[.\s]*)$/im;

  new Notice(`🚀 Запуск глобальной MOC-синхронизации v2.5...`, 2000);
