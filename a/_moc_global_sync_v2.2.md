...
================================================================================
 ...
 Версия: 2.2 (Code Block Exclusion)
...
--------------------------------------------------------------------------------
 ...
 !!! НОВОЕ в v2.2 !!!
 1. (ИСКЛЮЧЕНИЕ БЛОКОВ КОДА) Скрипт теперь полностью игнорирует содержимое
    многострочных (```) и однострочных (`) блоков кода при поиске ссылок.
    Это предотвращает ошибки, как на вашем скриншоте.
 2. (ГИБКИЙ ПОИСК ЗАГОЛОВКА) Сохранена возможность находить MOC-секцию,
    даже если слово "MOC" не является первым в заголовке.
================================================================================
*/
...
async function globalMocSyncV2(tp) {
  ...
  new Notice(`🚀 Запуск глобальной MOC-синхронизации v2.2...`, 2000);
  ...
  for (const source of mocSources) {
    ...
    // >>>>>>>>>> НОВЫЙ БЛОК: САНИТИЗАЦИЯ КОНТЕНТА <<<<<<<<<<
    let sanitizedContent = source.content;
    // 1. Удаляем многострочные блоки кода (```...```)
    sanitizedContent = sanitizedContent.replace(/```[\s\S]*?```/g, '');
    // 2. Удаляем однострочные блоки кода (`...`)
    sanitizedContent = sanitizedContent.replace(/`[^`]*`/g, '');
    // >>>>>>>>>> КОНЕЦ НОВОГО БЛОКА <<<<<<<<<<

    // Используем очищенный контент для дальнейшего парсинга
  ...
  if (globalParentMap.size === 0) {
    new Notice("ℹ️ Не найдено ни одной валидной ссылки в MOC-секциях (после очистки от кода).", 3000);
    return;
  }
