...
--------------------------------------------------------------------------------
 ...
 !!! НОВОЕ в v3.0 !!!
 1. (СОХРАНЕНИЕ СТРУКТУРЫ) Ключевое улучшение! При удалении всех родительских
    ссылок скрипт больше не удаляет свойство 'up'. Вместо этого он присваивает
    ему пустой список (up: []). Это сохраняет порядок полей в вашем
    frontmatter.
================================================================================
*/
...
async function globalMocSyncV3(tp) {
  ...
  new Notice(`🚀 Запуск глобальной MOC-синхронизации v3.0...`, 2000);
    ...
    await app.fileManager.processFrontMatter(childFile, (fm) => {
      ...
      if (JSON.stringify(uniqueNewParents) !== JSON.stringify(sortedCurrentUp)) {
        ...
        if (uniqueNewParents.length === 0) {
          fm.up = []; // Оставляем ключ, но делаем его пустым списком
        } else {
