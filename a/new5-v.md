...
// Получаем теги из frontmatter (они уже "чистые", без #)
let frontmatterTags = [];
...
// Убираем пустые значения на всякий случай
frontmatterTags = frontmatterTags.filter(Boolean);

// Получаем теги из тела заметки и СРАЗУ ЖЕ очищаем их от #
const bodyTags = parentMetadata?.tags?.map(t => t.tag.substring(1)) || [];

// Объединяем "чистые" теги из обоих источников, а затем удаляем дубликаты
